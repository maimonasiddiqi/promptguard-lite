...
import re
from utils import load_patterns,decode_payloads
from models import DetectionHit
@DetectorRegistry.register
class EncodingDetector(BaseDetector):
    def __init__(self):
        raw_info=load_patterns("patterns/encoding.json")
        self.pattern=[]
        for raw in raw_info:
    ...
    def scan(self, text: str) -> list[DetectionHit]:
        hits=[]
        decoded_dict = decode_payloads(text)
        variations = [text] + list(decoded_dict.values())
        for variant_text in variations:
            for pattern_info in self.pattern:
                for match in pattern_info['regex'].finditer(variant_text):
                    matched_text=match.group(0)
                    encoding_hit=DetectionHit(
